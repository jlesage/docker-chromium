...
This project provides a Docker container for [Chromium](https://www.chromium.org/Home/).
...
## Table of Contents
   ...
   * [Enable Usage of Sandbox](#enable-usage-of-sandbox)
...
[Releases]: https://github.com/jlesage/docker-chromium/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/chromium/tags
...
For Synology NAS users, follow these steps to update a container image:
  ...
  3.  In the search bar, type the name of the container (`jlesage/chromium`).
...
container, it would proxy all HTTP requests for `chromium.domain.tld` to
...
server {
	...
	location /chromium/ {
		...
		location ~ ^/chromium/(websockify(-.*)?) {
                        proxy_pass http://docker-chromium/$1;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of Chromium integrated into Docker image. |
...
Any desktop notification generated by Chromium is forwarded to the
...
## Enable Usage of Sandbox

For security purpose, Chromium uses sandbox to remove unnecessary
...
There are several ways to allow Chromium to use its sandbox.
From the most to least secure:
  ...
  1. **Use custom seccomp profile.** This is the most secure approach. Start
     ...
     parameter.
  2. **Add the `SYS_ADMIN` capability.** This allows Chromium to
...
> [!CAUTION]
> If none of the above actions are used, Chromium will run with
